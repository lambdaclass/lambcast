defmodule LambcastWeb.UserController do
  ...
  def show_cast(conn, %{"user" => user, "cast_hash" => cast_hash}) do
    fid = get_user_fid_by_name(user)
    user_name = get_user_fid_by_name(user)

    case Rpc.get_request("castsByParent?fid=#{user_name}&hash=#{cast_hash}&reverse=1") do
      %{"errCode" => code} ->
        conn
        |> put_flash(:error, "Error: #{code}")
        |> redirect(to: "/a")
      ...
      %{"messages" => messages} ->
        json(conn, %{"user" => user, "messages" => messages, "fid" => fid})
    end
  end

  defp get_user_fid_by_name(user) do
    case Rpc.get_request("userNameProofByName?name=#{user}") do
      %{"fid" => fid} ->
        fid
